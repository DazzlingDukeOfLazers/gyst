...
func (s *Store) SourceStates(ctx context.Context) ([]SourceStateRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.SourceID, &r.LocationKind, &r.CadenceSeconds, &r.PassStatus,
			&started, &r.PassDetail, &r.LatestObsID); err != nil {
			return nil, err
		}
		if started != nil {
...
func (s *Store) UpsertFinding(ctx context.Context, f FindingRow, now time.Time) (*string, error) {
	...
	if _, err := tx.Exec(ctx, `
		...
		f.FindingID, f.RuleID, f.RuleVersion, f.Severity, status, f.Subjects,
		f.Evidence, now, f.Confidence, f.Summary, nullableBytes(f.Remediation)); err != nil {
		return nil, err
	}
...
func (s *Store) FindingsForFile(ctx context.Context, sourceID, locator string) ([]FindingRow, error) {
	rows, err := s.pool.Query(ctx, selectFinding+`WHERE status <> 'resolved' ORDER BY severity, finding_id`)
	if err != nil {
		return nil, err
	}
